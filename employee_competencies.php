...
function loadEmployees() {
  fetch('get_employees.php')
    ...
    .then(data => {
      ...
      if (data.length > 0) {
        select.value = data[0].personal_info_id;   // ✅ match
        document.getElementById('assignBtn').disabled = false;
        loadEmployeeCompetencies(data[0].personal_info_id);  // ✅ match
      }
    })
...
function loadEmployeeCompetencies(empId) {
  fetch(`get_employee_competencies.php?personal_info_id=${empId}`)
    .then(res => res.json())
    .then(data => {
...
document.getElementById('assignBtn').addEventListener('click', async function() {
  const personalId = document.getElementById('employeeSelect').value;
  if (!personalId) return alert("Select an employee first.");

  try {
    // Get employee details (this returns employee_id too)
    const empRes = await fetch(`get_employee_details.php?personal_info_id=${personalId}`);
    const emp = await empRes.json();
    if (emp.error) return alert(emp.error);

    // 🔑 Assign the real employee_id (not personal_info_id) to hidden field
