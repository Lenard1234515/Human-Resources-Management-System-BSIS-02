...
<head>
   ...
   <style>
    ...
    /* Row highlight - higher specificity to override Bootstrap */
    table#cyclesTable tbody tr.selected-row {
        background-color: #d1ecf1 !important; /* light blue */
        color: #000 !important; /* ensure text is readable */
    }

    /* Keep striping on non-selected rows */
    table#cyclesTable tbody tr:not(.selected-row):nth-of-type(odd) {
        background-color: #f9f9f9; /* light gray */
    }

    table#cyclesTable tbody tr:not(.selected-row):nth-of-type(even) {
        background-color: #fff; /* white for even rows */
    }

    /* Optional: pointer cursor for rows */
    table#cyclesTable tbody tr {
        cursor: pointer;
    }
</style>
            ...
            if (Array.isArray(data) && data.length > 0) {
                data.forEach(ev => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>
                            <button class="btn btn-sm btn-warning edit-eval-btn" 
